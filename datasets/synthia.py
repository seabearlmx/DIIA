...
class Synthia(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            sobel = cv2.Sobel(gray, cv2.CV_16S, 1, 0)
            sobel = cv2.convertScaleAbs(sobel)
            sobel = np.asarray(sobel, np.float32)
        else:
            sobel = np.asarray(img, np.float32)
        ...
        return img, mask, img_name, mask_aux, sobel
    ...
    def build_epoch(self, cut=False):
        ...
        if self.class_uniform_pct > 0:
            if cut:
                ...
                self.imgs_uniform = uniform.build_epoch_2(self.imgs,
                                                        self.fine_centroids,
                                                        num_classes,
                                                        cfg.CLASS_UNIFORM_PCT)   # build_epoch
            else:
                self.imgs_uniform = uniform.build_epoch_2(self.imgs + self.aug_imgs,
                                                        self.centroids,
                                                        num_classes,
                                                        cfg.CLASS_UNIFORM_PCT)   # build_epoch
        else:
