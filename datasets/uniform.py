...
def class_centroids_all_from_color(items, num_classes, id2trainid, tile_size=1024):
    """
...
def build_epoch_2(imgs, centroids, num_classes, class_uniform_pct):
    """
    Generate an epochs-worth of crops using uniform sampling. Needs to be called every
    imgs: list of imgs
    centroids:
    ...
    for class_id in range(num_classes):
        # # synthia
        if class_id == 9 or class_id == 14 or class_id == 16:
            continue
        # # synthia
        string_format = "cls %d len %d"% (class_id, len(centroids[class_id]))
        logging.info(string_format)
