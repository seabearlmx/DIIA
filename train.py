...
def main():
    ...
    net = network.get_net(args, criterion, criterion_aux, L1_loss)
...
def train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter):
    ...
    for i, data in enumerate(train_loader):
        ...
        inputs, gts, _, aux_gts, boundary_gts = data
        ...
        if len(inputs.shape) == 5:
            ...
            gts = [gts]
            aux_gts = [aux_gts]
            boundary_gts = [boundary_gts]

        batch_pixel_size = C * H * W

        for di, ingredients in enumerate(zip(inputs, gts, aux_gts, boundary_gts)):
            input, gt, aux_gt, boundary_gt = ingredients

            start_ts = time.time()

            img_gt = None
            input, gt, aux_gt, boundary_gt = input.cuda(), gt.cuda(), aux_gt.cuda(), boundary_gt.cuda()

            optim.zero_grad()
            if args.use_isw:
                outputs = net(input, gts=gt, aux_gts=aux_gt, img_gt=img_gt, boundary_gt=boundary_gt, visualize=args.visualize_feature,
            ...
            outputs_index += 1
            loss_final = outputs[outputs_index]
            outputs_index += 1
            loss_struct = outputs[outputs_index]
            outputs_index += 1
            loss_boundary = outputs[outputs_index]
            outputs_index += 1
            loss_l1_boundary = outputs[outputs_index]
            outputs_index += 1

            if loss_struct > 3:
                loss_struct = 0.01 * loss_struct

            total_loss = 0.5 * main_loss + 0.1 * aux_loss + loss_final + 0.1 * loss_boundary + 0.1 * loss_struct + 0.001 * loss_l1_boundary
            ...
            main_loss_value = 0.5 * main_loss.item()
            aux_loss_value = 0.1 * aux_loss.item()
            loss_final_value = loss_final.item()
            loss_boundary_value = 0.1 * loss_boundary.item()
            loss_struct_value = 0.1 * loss_struct.item()
            loss_l1_boundary_value = 0.001 * loss_l1_boundary.item()
            ...
            if args.local_rank == 0:
                if i % 50 == 49:
                    ...
                    msg = '[epoch {}], [iter {} / {} : {}], [loss {:0.6f}], [main_loss_value {:0.6f}], ' \
                          '[aux_loss_value {:0.6f}],[loss_final_value {:0.6f}],[loss_boundary_value {:0.6f}],' \
                          '[loss_struct_value {:0.6f}],[loss_l1_boundary_value {:0.6f}],[lr {:0.6f}], [time {:0.4f}]'.format(
                        curr_epoch, i + 1, len(train_loader), curr_iter, train_total_loss.avg, main_loss_value,
                        aux_loss_value,
                        loss_final_value, loss_boundary_value, loss_struct_value, loss_l1_boundary_value,
...
def validate(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, save_pth=True):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        inputs, gt_image, img_names, _, _ = data
        ...
        with torch.no_grad():
            if args.use_wtloss:
                main_output, final_output, f_cor_arr = net(inputs, visualize=True)
            else:
                main_output, final_output = net(inputs)
            output = 0.5 * main_output + final_output
